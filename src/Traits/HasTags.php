...
trait HasTags
{
  ...
  {
      static::created(function (Model $elementToTag) {
          if (count($elementToTag->queuedTags) === 0) {
              return;
          }
  ...
  protected function syncTagIds($ids, string | null $type = null, $detaching = true): void
  {
      ...
      $current = $this->tags()
          ->newPivotStatement()
          ->where(config('hive-posts.column_names.tags_morph_id'), $this->getKey())
          ->where(config('hive-posts.column_names.tags_morph_type'), $this->getMorphClass())
          ...
          ->pluck(config('hive-posts.column_names.tags_morph_id'))
          ->all();
