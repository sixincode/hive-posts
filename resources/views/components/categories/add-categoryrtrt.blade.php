<div>
 ...
 <div class="grid space-y-2">
  <div class="flex flex-auto flex-wrap">
    <div class="w-full" x-data="categoriesSelect">
       ...
       <div class="w-full">
           <div class="grid flex-auto flex-wrap" x-on:click="open">
                 ...
                 <div class="mt-2 relative rounded-md shadow-sm">
                     ...
                     <input type="text" name="search" autocomplete="off" id="search" x-model.debounce.750ms="search" class="focus:ring-sky-500 focus:border-sky-500 block w-full pl-10 sm:text-sm border border-sky-300 rounded-md h-10" placeholder="" @keyup.escape="clear" @keyup.delete="deselect">
                             ...
                             <template x-for="(option,index) in options" :key="option.text">
                                 <li class="text-gray-900 cursor-default select-none relative" role="option">
                                     <div class="cursor-pointer w-full border-gray-100 rounded-t border-b hover:bg-slate-100" x-bind:class="option.selected ? 'bg-blue-100' : ''" @click="select(index,$event)">
                                         <div x-bind:class="option.selected ? 'border-sky-600' : ''" class="flex w-full items-center p-2 pr-4 border-transparent border-l-2 relative">
                                             <div class="w-full items-center flex">
                                                 <div class="mx-2 truncate leading-6" x-model="option" x-text="option.text"></div>
                                                 <span class="absolute inset-y-0 right-0 flex items-center pr-2 text-sky-600" x-show="option.selected">
         ...
         <div class="">
             ...
             <template x-for="(option,index) in selectedCategories" :key="option.value">
                 <div x-show="index < 3"
                     class="flex justify-center items-center m-1 font-medium py-1 px-2 rounded-full text-sky-700 bg-sky-100 border border-sky-300 ">
             ...
             <div x-show="selectedCategories.length > 3" class="flex justify-center items-center m-1 font-medium py-1 px-2 rounded-full text-sky-700 bg-sky-100 border border-sky-300 ">
                 <div class="text-xs font-normal h-6 flex justify-center items-center leading-none max-w-full flex-initial">
                     <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-sky-200 text-pink-800 mr-2">
...
document.addEventListener("alpine:init", () => {
   Alpine.data("categoriesSelect", (obj) => ({
       elementId: 'categoriesOptions',
       options: [],
       selected: @entangle('categories'),
       selectedCategories: [],
       // selectedCategories:  @entangle('categories'),
       show: false,
       search: '',
       open() {
           this.show = true
       ...
       select(index, event) {
           if (!this.options[index].selected) {
             ...
             // this.selectedCategories.splice(this.options[index].value, 1);
